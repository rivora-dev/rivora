...
/// Shared Workspace launcher used by `rivora-workspace` and bare `rivora`.
///
/// Owns Runtime/Capability bootstrap, interactive vs smoke mode selection,
/// and TTY gating for interactive sessions. Callers must not reimplement this path.
pub fn run_workspace(config: WorkspaceLaunchConfig) -> Result<(), String> {
    ...
    let caps = open_capabilities(&config.data_dir)?;
    run_interactive_workspace(&caps)
}

/// True when stdin and stdout are interactive terminals.
...
fn run_interactive_workspace(caps: &CapabilityService) -> Result<(), String> {
    ...
    // Restore terminal-ish cleanliness after dialoguer sessions.
    println!("Goodbye.");
    Ok(())
}
