...
tar -czf "${TEST_INSTALL}/rivora-v0.9.1-test.tar.gz" -C "${TEST_INSTALL}/pkg" rivora LICENSE README.md
mkdir -p "${TEST_INSTALL}/dest"
install_from_archive "${TEST_INSTALL}/rivora-v0.9.1-test.tar.gz" "${TEST_INSTALL}/dest" "${TEST_INSTALL}/extract"
assert_eq "installed binary exists" "1" "$([ -x "${TEST_INSTALL}/dest/rivora" ] && echo 1 || echo 0)"
assert_contains "installed version" "0.9.1" "$("${TEST_INSTALL}/dest/rivora" --version)"

mkdir -p "${TEST_INSTALL}/nowrite"
chmod 555 "${TEST_INSTALL}/nowrite"
assert_fails "non-writable install dir" install_from_archive "${TEST_INSTALL}/rivora-v0.9.1-test.tar.gz" "${TEST_INSTALL}/nowrite" "${TEST_INSTALL}/extract2"
...
if [ -f "${ROOT}/target/release/rivora" ]; then
    ...
    case "$binver" in
        *0.9.1*)
            ...
            assert_ok "package-release" "${ROOT}/scripts/package-release.sh" v0.9.1 aarch64-apple-darwin "${ROOT}/target/release" "$TEST_OUTPKG"
            assert_eq "archive present" "1" "$([ -f "${TEST_OUTPKG}/rivora-v0.9.1-aarch64-apple-darwin.tar.gz" ] && echo 1 || echo 0)"
            ;;
        *)
            pass "package-release skip (binary not 0.9.1 yet)"
            ;;
    esac
else
